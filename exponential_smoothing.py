...
# initialize level with average for first several periods
...
# initialize level
# initialize trend
def Holt_smoothing_damped(series, alpha, beta, phi):
    result = [series[0]] # first value is same as series
    for n in range(1, len(series)+1):
        if n == 1:
            level, trend = series[0], series[1] - series[0]
        if n >= len(series): # we are forecasting
          ...
          value = series[n]
        last_level, level = level, alpha*value + (1-alpha)*(level+phi*trend)
...
# initialize seasonal:
# slen: season_length (P in my notation)
# compute season_average
